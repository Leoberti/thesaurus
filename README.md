...
- 🎯 Entry points to execute your program using `python -m <thesaurus>` or `$ thesaurus` with basic CLI argument parsing.
...
---
# thesaurus Flask Application

Awesome thesaurus created by Leoberti
...
```bash
git clone https://github.com/Leoberti/thesaurus thesaurus
cd thesaurus
make install
```
...
```bash
thesaurus create-db   # run once
thesaurus populate-db  # run once (optional)
thesaurus add-user -u admin -p 1234  # ads a user
thesaurus run
```

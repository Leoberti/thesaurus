...
original_author="Leoberti"
original_name="thesaurus"
original_urlname="thesaurus"
original_description="Awesome thesaurus created by Leoberti" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/thesaurus "${name}"
...
then
    ...
    echo "then run 'thesaurus' or 'python -m thesaurus'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'thesaurus' or 'python -m thesaurus' or 'poetry run thesaurus'"
fi
